"""Module for generating crosswords"""
import random
import requests
import json
import re
...
class CrossGen():
    ...
    def __init__(self, size):
        """Loads words from the dictionary 'words.txt'. 
        
        Parameters
        ----------
        size : int
            The number of words in each row/column that there will be. 
            Larger number means lower probability of generating a valid crossword
            3 takes a second or two; 4 takes a couple seconds; 5 is slow; 6 is like impossible
        """
        print ("Loading words...")
    ...
    def get_words(self, chars)->dict|list[str]:
        ...
        ----------
        chars : str
            get_words will return all words that have this substring of characters. 
    ...
    def to_str_array(self, cross: list[list, list])->tuple[int, int, int, list[str]]:
        """Returns a 2d array of characters that is a representation of a finished crossword. 
        Underscore for no character in that location. 

        Parameters
        ----------
        list[list, list]
            A crossword that was generated by __generate_cross or generate_cross
        
        Returns
        -------
        tuple
            int
                The x spacing until the beginning of the 'down' words
            int
                The y spacing until the beginning of the 'across' words
            int
                The number of down/across words
            list[str]
                The 2d char array of a finished crossword, where an underscore is no character in that location. 
        """
        to_ret = [""]
    ...
    def get_hint(self, word)->list[str, str]:
        ...
        list[
            str
                The type of hint (wikipedia, synonym, etc)
            str
                The hint (up to 200 characters long)
        ]
        """
        try:
